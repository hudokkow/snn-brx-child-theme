...
function snn_security_section_callback() {
    echo '<p class="description">' . esc_html__( 'Configure your security settings below:', 'snn' ) . '</p>';
}
...
function snn_math_captcha_callback() {
    ...
    <div class="setting-wrapper">
        <input id="enable_math_captcha" type="checkbox" name="snn_security_options[enable_math_captcha]" value="1" <?php checked(isset($options['enable_math_captcha']) && $options['enable_math_captcha'], 1); ?>>
        <label for="enable_math_captcha" title="hide_element">Enable Math Captcha</label>
        <p class="description"><?php esc_html_e( 'Adds a math captcha challenge on the login page to improve security.', 'snn' ); ?></p>
    </div>
    <?php
}
?>
